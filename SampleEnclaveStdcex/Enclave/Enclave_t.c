...
static sgx_status_t SGX_CDECL sgx_ecall_memset_s(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_memset_s_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_memset_s_t* ms = SGX_CAST(ms_ecall_memset_s_t*, pms);
	ms_ecall_memset_s_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_memset_s_t), ms, sizeof(ms_ecall_memset_s_t))) {
...
static sgx_status_t SGX_CDECL sgx_ecall_fchmod(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_fchmod_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_fchmod_t* ms = SGX_CAST(ms_ecall_fchmod_t*, pms);
	ms_ecall_fchmod_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_fchmod_t), ms, sizeof(ms_ecall_fchmod_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	int _in_retval;
...
static sgx_status_t SGX_CDECL sgx_ecall_socket_receiver(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	ecall_socket_receiver();
	return status;
}

static sgx_status_t SGX_CDECL sgx_ecall_socket_sender(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	ecall_socket_sender();
...
static sgx_status_t SGX_CDECL sgx_ecall_time(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_time_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_time_t* ms = SGX_CAST(ms_ecall_time_t*, pms);
	ms_ecall_time_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_time_t), ms, sizeof(ms_ecall_time_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	int _in_retval;
...
static sgx_status_t SGX_CDECL sgx_ecall_mmap(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_mmap_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_mmap_t* ms = SGX_CAST(ms_ecall_mmap_t*, pms);
	ms_ecall_mmap_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_mmap_t), ms, sizeof(ms_ecall_mmap_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	int _in_retval;
...
SGX_EXTERNC const struct {
	...
	7,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_memset_s, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_fchmod, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_socket_receiver, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_socket_sender, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_time, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_mmap, 0, 0},
	}
};

SGX_EXTERNC const struct {
	size_t nr_ocall;
	uint8_t entry_table[113][7];
} g_dyn_entry_table = {
	113,
	{
		{0, 0, 0, 0, 0, 0, 0, },
